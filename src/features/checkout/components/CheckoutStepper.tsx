...
export default function CheckoutStepper({ currentStep, steps }: CheckoutStepperProps) {
  return (
    <div className="flex items-center justify-between max-w-4xl mx-auto relative px-4">
      ...
      <motion.div className="absolute top-1/2 left-0 h-1 bg-green-500 -translate-y-1/2 z-0 rounded-full"
        initial={{ width: "0%" }}
      ...
      {steps.map((step) => (
        <div key={step.id} className="relative z-10 flex flex-col items-center gap-3">
          <motion.div
            ...
            className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-lg transition-colors border-4 ${currentStep >= step.id ? "border-green-100 dark:border-green-900/30" : "border-white dark:border-gray-900"}`}>
            {currentStep > step.id ? <CheckCircle2 className="w-6 h-6" /> : <step.icon className="w-5 h-5" />}
          </motion.div>
          <span className={`text-[10px] font-black uppercase tracking-widest hidden md:block ${currentStep >= step.id ? "text-green-600 dark:text-green-500" : "text-gray-400"}`}>
            {step.name}
          </span>
        </div>
      ))}
