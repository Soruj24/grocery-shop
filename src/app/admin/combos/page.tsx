...
export default function AdminCombosPage() {
  ...
  const columns = [
    ...
    { key: "saveAmount", label: "You Save", render: (item: any) => <span className="text-xs font-semibold text-success">৳{Number(item.saveAmount).toLocaleString()}</span> },
    { key: "tag", label: "Tag", render: (item: any) => <span className="text-[10px] font-bold uppercase px-2 py-0.5 rounded-full bg-success-subtle text-success">{item.tag}</span> },
    ...
    { key: "actions", label: "", render: (_: any, i: number) => (
      <div className="flex items-center gap-1">
        <button onClick={() => window.location.href = `/admin/combos/${combos[i]._id}`} className="p-1.5 rounded-lg text-muted-foreground hover:bg-muted hover:text-foreground"><svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg></button>
        <button onClick={() => { if (confirm("Delete combo?")) del(combos[i]._id); }} className="p-1.5 rounded-lg text-muted-foreground hover:bg-danger-subtle hover:text-danger"><Trash2 className="h-3.5 w-3.5" /></button>
      </div>
    )},
  ];
  ...
  return (
    <div className="space-y-6">
      <AdminPageHeader title="Combo Packs" description="Manage product bundles and combo offers"
        actions={<button onClick={() => window.location.href = "/admin/combos/new"} className="flex items-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-semibold text-white hover:bg-primary/90"><Plus className="h-4 w-4" /> Add Combo</button>}
      />
