    ...
    <div className="space-y-8">
              ...
              {topProducts.slice(0, 5).map((p: any, i: number) => (
                <div key={String(p._id)} className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between mb-1"><span className="text-sm font-medium text-foreground">{p.name as string}</span><span className="text-xs font-bold">৳{Number(p.revenue).toLocaleString()}</span></div>
                    <div className="h-1.5 rounded-full bg-muted overflow-hidden"><div className="h-full rounded-full bg-primary" style={{ width: `${Math.min(100, (Number(p.totalSold) / 10) * 100)}%` }} /></div>
                  </div>
                  <span className="text-[10px] text-muted-foreground w-12 text-right">{String(p.totalSold || 0)} sold</span>
                </div>
              ))}
            </div>
          )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="rounded-xl border border-border bg-card p-5">
          ...
          {isLoading ? <div className="h-[200px] rounded-lg bg-muted animate-pulse" /> : (
        ...
        <div className="lg:col-span-2 rounded-xl border border-border bg-card p-5">
          <h3 className="text-sm font-semibold text-foreground mb-4">User Growth (30 Days)</h3>
          {isLoading ? <div className="h-[200px] rounded-lg bg-muted animate-pulse" /> : (
            <ResponsiveContainer width="100%" height={200}>
