  ...
  const columns = [
    { key: "code", label: "Code", render: (item: any) => (
      <div className="flex items-center gap-3">
        <div className="h-8 w-8 rounded-lg bg-success-subtle flex items-center justify-center"><Tag className="h-4 w-4 text-success" /></div>
        <span className="text-sm font-mono font-bold text-foreground">{item.code}</span>
      </div>
    )},
    ...
    { key: "minOrderAmount", label: "Min Order", render: (item: any) => <span className="text-xs text-muted-foreground">৳{Number(item.minOrderAmount).toLocaleString()}</span> },
    { key: "expiryDate", label: "Expires", render: (item: any) => <span className="text-xs text-muted-foreground">{new Date(item.expiryDate).toLocaleDateString()}</span> },
    ...
    { key: "actions", label: "", render: (_: any, i: number) => (
      <div className="flex items-center gap-1">
        <button onClick={() => setModal({ open: true, data: coupons[i] })} className="p-1.5 rounded-lg text-muted-foreground hover:bg-muted hover:text-foreground"><svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg></button>
        <button onClick={() => { if (confirm("Delete coupon?")) del(coupons[i]._id); }} className="p-1.5 rounded-lg text-muted-foreground hover:bg-danger-subtle hover:text-danger"><Trash2 className="h-3.5 w-3.5" /></button>
      </div>
    )},
  ];
  ...
  return (
    <div className="space-y-6">
      <AdminPageHeader title="Coupons" description="Manage discount coupons"
        actions={<button onClick={() => setModal({ open: true })} className="flex items-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-semibold text-white hover:bg-primary/90"><Plus className="h-4 w-4" /> Add Coupon</button>}
      />
