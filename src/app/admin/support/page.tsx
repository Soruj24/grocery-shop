...
export default function AdminSupportPage() {
  ...
  const statusColors: Record<string, string> = {
    open: "bg-success-subtle text-success", pending: "bg-warning-subtle text-warning",
    resolved: "bg-muted text-muted-foreground", closed: "bg-danger-subtle text-danger",
  };
  const priorityColors: Record<string, string> = {
    low: "bg-primary/10 text-primary", medium: "bg-warning-subtle text-warning",
    high: "bg-danger-subtle text-danger", urgent: "bg-danger-subtle text-danger",
  };
  ...
  const columns = [
    ...
    { key: "subject", label: "Subject", render: (item: Record<string, unknown>) => <span className="text-sm text-muted-foreground">{item.subject as string}</span> },
    { key: "priority", label: "Priority", render: (item: Record<string, unknown>) => (
      <span className={cn("text-[10px] font-bold uppercase px-2 py-0.5 rounded-full", priorityColors[item.priority as string] || "")}>{item.priority as string}</span>
    )},
    { key: "status", label: "Status", render: (item: Record<string, unknown>) => (
      <select value={item.status as string} onChange={(e) => update({ id: item._id as string, body: { status: e.target.value } })}
        className={cn("text-[10px] font-bold uppercase px-2 py-0.5 rounded-full border-0 outline-none cursor-pointer", statusColors[item.status as string] || "")}>
        {Object.keys(statusColors).map((s) => <option key={s} value={s}>{s}</option>)}
      </select>
    )},
    { key: "actions", label: "", render: (item: Record<string, unknown>) => (
      <button onClick={() => setReplyModal(item)} className="p-1.5 rounded-lg text-muted-foreground hover:bg-muted hover:text-foreground"><MessageSquare className="h-3.5 w-3.5" /></button>
    )},
  ];
    ...
    <div className="space-y-6">
      ...
      {replyModal && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => { setReplyModal(null); setReplyText(""); }}>
          <div className="bg-card rounded-xl w-full max-w-lg p-6" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg font-bold mb-1">Reply to Ticket</h3>
            <p className="text-sm text-muted-foreground mb-4">Customer: {replyModal.customer as string} — {replyModal.subject as string}</p>
            <textarea value={replyText} onChange={(e) => setReplyText(e.target.value)} placeholder="Type your reply..." rows={4}
              className="w-full rounded-xl border border-border p-3 text-sm outline-none focus:border-primary resize-none" />
            <div className="flex gap-2 mt-4 justify-end">
              <button onClick={() => { setReplyModal(null); setReplyText(""); }} className="px-4 py-2 rounded-xl border text-sm font-medium text-muted-foreground hover:bg-muted">Cancel</button>
              ...
              }} className="px-4 py-2 rounded-xl bg-primary text-sm font-semibold text-white hover:bg-primary/90">Send Reply</button>
            </div>
          </div>
        </div>
      )}
