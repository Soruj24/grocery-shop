  ...
  if (!order || (order as { error?: string }).error) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center space-y-4">
        <h2 className="text-2xl font-bold text-foreground">{t("order_not_found")}</h2>
        <p className="text-muted-foreground">{t("order_not_found_desc")}</p>
        <Link href="/orders" className="text-primary font-bold hover:underline">{t("back_to_order_list")}</Link>
      </div>
    );
  }
      ...
      <div className="relative z-10 space-y-12">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2">
            <Link href="/orders" className="flex items-center gap-2 text-primary font-bold hover:gap-3 transition-all mb-4">
              <ArrowLeft className="w-4 h-4" /> {t("go_back")}
            </Link>
            <h1 className="text-4xl font-black text-foreground tracking-tight">{t("order_tracking_label")}</h1>
            <p className="text-muted-foreground font-bold">
              {t("order_id_label")}: <span className="text-foreground font-black">#{order._id.slice(-8).toUpperCase()}</span>
            </p>
          </div>
          <div className="bg-primary text-primary-foreground px-6 py-3 rounded-2xl shadow-primary flex items-center gap-3">
